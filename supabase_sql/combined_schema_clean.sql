...
GROUP BY du.session_id, tf.server_software, tf.programming_language, tf.framework, tf.cms;
...
RETURNS DECIMAL AS $$
BEGIN
    IF impact_score IS NULL OR exploitability_score IS NULL THEN
        RETURN NULL;
    END IF;
    
    -- Risk = (Impact × Exploitability × Confidence) / 10
    RETURN ROUND((impact_score * exploitability_score * confidence) / 10, 2);
END;
$$ language 'plpgsql';
